...
var connectBlockchain = async () => {
    ...
    // console.log(JSON.parse(JSON.stringify(dummyDegree)));
    var addedToBlockchain = await CreateDegree(contract, JSON.stringify(dummyDegree))
    console.log("result from invoke blockchain")
    console.log(JSON.parse(addedToBlockchain.toString()))
    console.log("done adding dummy degree to blockchain")

    var queryResult = await QueryDegree(contract, String(dummyDegree._id));
    var queryResultJSON = JSON.parse(queryResult.toString())
    console.log("query blockchain result:");
    console.log(queryResultJSON)

    const hashString = hasher.hash(JSON.parse(JSON.stringify(dummyDegree)))
    ...
    University.findById(universityId)
        ...
        .exec(function (err, foundUniversity) {
            ...
            var dummyDegree = {
                ...
                cgpa: 0,
                honors: 'dummy',
                issueDate: req.body.issueDate,//so mongo can apply its date formatting
                universityName: 'dummy'
            }
            Degree.create(dummyDegree, async function (err, savedDummyDegree) {
                if (err) {
                    ...
                    await Degree.findOneAndUpdate({ _id: savedDummyDegree._id }, { $set: newDegree }, { new: true }, async function (err, savedFinalDegree) {
                        ...
                        console.log("final degree saved to DB: ")
                        console.log(savedFinalDegree);
...
app.get("/degrees/:id", middleware.isLoggedIn, middleware.canViewDegree, async function (req, res) {
    ...
    Degree.findById(id, function (err, foundDegree) {
        if (err) {
            ...
            //Dumb: chancellor and dean is in university model
            University.findById(foundDegree.university, async function (err, foundUniversity) {
                if (err) {
                    console.log(err)
                    req.flash('error', 'A database error occurred. Please try again later')
                    return res.redirect("/" + req.user.userType.toLowerCase() + "/" + req.user.userObject)
                ...
                console.log("server is hashing degree from database: ");
                console.log(foundDegree)
                const hashString = String(hasher.hash(JSON.parse(JSON.stringify(foundDegree))))
                console.log("server hashing result: " + hashString)
                if (hashString.toString() == queryResultJSON.hash) {
                    console.log("HASHES MATCH")
                    res.render("viewDegree", { degree: foundDegree, university: foundUniversity, serverHash: hashString });
                }
                else {
                    console.log("HASHES DONT MATCH");
                    req.flash("error","Hash of database degree doesn't match blockchain. The degree has been illegally modified. Contact your university to resolve the issue")
                    res.redirect("/" + req.user.userType.toLowerCase() + "/" + req.user.userObject)                }
            })
        }
    })
